...
module.exports.run = async function ({ api: b, event: a, args: c }) {
    ...
    if (0 == c.length) return f("Yes speak dear (ღ˘⌣˘ღ)");
    switch (c[0]) {
        case "on":
            return global.simsimi.has(d) ? f("Sim has not turned off.") : (global.simsimi.set(d, e), f("Sim On success."));
        case "off":
            return global.simsimi.has(d) ? (global.simsimi.delete(d), f("Sim is Off.")) : f("Sim has not turned on");
        default:
