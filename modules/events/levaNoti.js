...
module.exports.run = async function({ api, event, Users, Threads }) {
  ...
  const hours = moment.tz("Asia/Dhaka").format("HH");
	...
	const type = (event.author == event.logMessageData.leftParticipantFbId) ? "Goodbyee" : "😭";
	const path = join(__dirname, "cache", "leaveGif");
...
(typeof data.customLeave == "undefined") ? msg = "[✨] {name} Left us : ) . {type}  [✨]\n●▬▬▬▬๑۩۩๑▬▬▬▬●\n🌺🌸🌺 Thats good no need to kick <3😊💔\n\n[❤️‍🔥] Remember us baby. 𝙃𝙖𝙫𝙚 𝙖 𝙃𝙖𝙥𝙥𝙮 Life. {session} || {time}" : msg = data.customLeave;
